...
3) Должны быть минимальными, но полными: не стоит засорять имена классов/переменных ненужными дополнениями по типу `Info`, `Data`, `Handler`, `Class`, `Table` и т.д. Даже, если класс действительно что-то обрабатывает, зачастую эту приписку лучше не добавлять. Данные вещи можно добавлять, только если уверены, что читающий ваш код точно поймет, в чем разница между `message` и  `messageHandler`.
